...
outputunit::outputunit(const string probname, const string datadir, const int nt, const int tm_in, const int tp_in, const int ts_in, const int xm_in[3], const int xp_in[3], const int xs_in[3],
    ...
    if (field_in == "vx" || field_in == "vy" || field_in == "vz" || field_in == "sxx" || field_in == "sxy" || field_in == "sxz" || field_in == "syy" ||
        ...
        for (int i=0; i<nifaces; i++) {
            ...
            if ((direction == 0) && (xm_in[0] == xp_in[0])) {
            ...
            } else if ((direction == 1) && (xm_in[1] == xp_in[1])) {
            ...
            } else if ((direction == 2) && (xm_in[2] == xp_in[2])) {
        ...
        // if needed, check if interface has state variable
        if (field_in == "state" && !d.interfaces[location]->has_state) {
            cerr << "Interface for output does not have a state variable in outputunit.cpp\n";
            MPI_Abort(MPI_COMM_WORLD, -1);
        }
    }
    ...
    // create communcator for appropriate processes
